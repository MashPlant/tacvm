...
pub struct RunConfig {
  pub inst_limit: u32,
  pub stack_limit: u32,
  // if vm returns with an error, stacktrace will be printed
...
impl RunConfig {
  pub fn simple() -> Self {
    Self::default_io(!0, !0, false) // no limit, no stacktrace
  }

  pub fn default_io(inst_limit: u32, stack_limit: u32, print_stacktrace: bool) -> Self {
    Self { inst_limit, stack_limit, print_stacktrace, vm_input: Box::new(BufReader::new(io::stdin())), vm_output: Box::new(io::stdout()), info_output: Box::new(io::stderr()) }
  }
}
...
impl VM<'_> {
  ...
  pub fn new<'a>(p: &'a Program<'a>) -> VM<'a> {
    ...
    VM { pc: 0, mem, stack: vec![], program: p, str_addr, vtbl_addr }
  }
