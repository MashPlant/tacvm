...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  let matches = App::new("tacvm").author("MashPlant").about("A naive virtual machine to execute tac code, for compiler course's use.")
    ...
    .arg(Arg::with_name("stacktrace").long("stacktrace"))
    .arg(Arg::with_name("inst_count").long("inst_count"))
  ...
  let stacktrace = matches.is_present("stacktrace");
  let inst_count = matches.is_present("inst_count");
  ...
  Ok(tacvm::work(&code, inst_limit, stack_limit, stacktrace, inst_count, vm_input, vm_output, info_output)?)
}